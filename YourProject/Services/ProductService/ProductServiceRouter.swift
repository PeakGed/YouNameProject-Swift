...
enum ProductServiceRouter: AlamofireBaseRouterProtocol {
    ...
    case fetchByQuery(request: ProductServiceRequest.FetchByQuery)
    case fetchByCategory(request: ProductServiceRequest.FetchByCategory)
    ...
    var path: String {
        switch self {
        case .fetchByHotel, .fetchByQuery, .fetchByCategory:
            return "/v4/products"
    ...
    var method: Alamofire.HTTPMethod {
        switch self {
        case .fetchByHotel, .fetchByQuery, .fetchByCategory, .fetchById:
            return .get
        case .createProduct:
            return .post
        case .updateProduct:
    ...
    var parameters: [String: Any]? {
        ...
        case .fetchByQuery(let request):
            return request.parameters
        case .fetchByCategory(let request):
            return request.parameters
        default:
            return nil
        }
